...
public class Solution {
    ...
    private static void dfs(int depth, int[][] currentMap) {
        ...
        for(int w = 0; w < W; w++) { // 고를 수 있는 숫자는 0부터 W까지
            perm[depth] = w;
             
            int[][] copy = new int[H][W];
             
            for (int i = 0; i < H; i++) {
                copy[i] = currentMap[i].clone();
            }
    ...
    private static void gravity(int[][] currentMap) {
        for (int i = 0; i < W; i++) {
            ...
            int index = H-1;
            for (int j = H-1; j >= 0; j--) {
                if(currentMap[j][i] != 0) { // 현재 확인하고 있는 위치가 0이 아니면
                    tmp = currentMap[j][i]; // 벽돌 번호 잠시 적어두고
                    currentMap[j][i] = 0; // 현재 위치 지우기
                    currentMap[index][i] = tmp; // index 위치에 쓰기
                    index--;
                }
            }
        }
    ...
    private static void bfs(int[] start, int[][] currentMap) {
        int start_h = start[0];
        int start_w = start[1];
        ...
        while(!dq.isEmpty()) {
            ...
            int range = currentMap[cur_h][cur_w]-1; // 2의 좌표에는 값이 2이므로 범위를 알 수 있다.
            for (int d = 0; d < 4; d++) { // 상 하 좌 우 동일하게 적용
                ...
                for (int i = 0; i < range; i++) {
                    ...
                    if (currentMap[nh][nw] == 0) continue; // 범위에 포함되어도 0이면 큐에 넣지 않는다
                    dq.addLast(new int[] {nh, nw}); // 자기 범위 내의 상 하 좌 우 중 0이 아닌 것들의 좌표 큐에 삽입
                }
            }
             
            currentMap[cur_h][cur_w] = 0; // 다 끝났으면 자기 위치는 0으로 만들기
        }
    }
}